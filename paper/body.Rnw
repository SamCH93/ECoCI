...
to the prior standard deviation $\sigma_\theta$.

In the following, we will discuss how different prior means $\mu_\theta$ and
standard deviations $\sigma_\theta$ affect the resulting support intervals. When
the prior standard deviation decreases ($\sigma_\theta \downarrow 0$), the prior
approaches a point mass at $\mu_\theta$. The width of the support interval is
then fully determined by the difference between the parameter estimate $\that$
and the prior mean $\mu_\theta$ divided by the standard error $\sigma$. A
smaller difference between $\that$ and $\mu_{\theta}$ leads to a tighter support
interval. In contrast, for priors that become increasingly diffuse
($\sigma_\theta \to \infty$), the $k \geq 1$ support interval~\eqref{eq:si}
...
\emph{normal moment priors}
$\theta \sim \mathrm{NM}(\mu_\theta, \sigma_\theta^2)$, which have density
$f(\theta \given \mu_\theta, \sigma_\theta^2) = \Nor(\theta \, ; \, \mu_\theta, \sigma_\theta^2) \times (\theta - \mu_\theta)^{2}/\sigma_\theta^2$
where $\Nor(\cdot \, ; \, \mu_\theta, \sigma_\theta^2)$ denotes the density
