...
\caption{Comparison of different support intervals for data from RECOVERY trial.
  ...
  unit variance $\sigma_\theta^{2} = \Sexpr{round(v, 2)}$. The scale
...
support interval can be read of from it. If there are space constraints,
%(for instance, when one computes support
%intervals for a number of regression coefficients),
a compromise is to report support intervals for different levels (\eg{}
