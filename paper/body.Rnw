...
To fully specify the support interval~\eqref{eq:silocal}, the prior variance
...
% Depending on whether the Bayes factor is oriented in favor of the null $\h{0}$
% (as here) or the alternative $\h{1}$, such Bayes factor bounds are called
% \emph{minimum Bayes factors} or \emph{maximum Bayes factors}, respectively.
...
cheating \citep{Berger1987}. This is reflected by the fact that for a given
estimate (i.e., data set) and fixed support level $k$, the interval represents
the narrowest support interval possible among all possible support intervals.
When minimizing over the class of all two-parameter normal priors, i.e., the
Bayes factor~{\eqref{eq:bf01}}, we get the same minimum Bayes
...
normal prior. The scale parameter of the nonlocal moment prior $\sigma_{\theta}$
is elicited with a similar approach as in \citet{Pramanik2022}; The value
...
\caption{Comparison of different support intervals for data from RECOVERY trial.
  ...
  unit variance of $\sigma_\theta^{2} = \Sexpr{round(v, 2)}$. The scale
  parameter of the nonlocal normal moment prior is
...
\begin{align}
  \label{eq:existence}
  \log(1 + \sigma_\theta^2/\sigma^2) + \frac{(\that - \mu_{\theta})^{2}}{\sigma^2 + \sigma_\theta^2} \geq 2 \log k.
\end{align}
...
sample size $n_2$ other support intervals with greater support levels $k$ can be
...
$(1 - k)\%$ confidence interval.
% This is the price that has to be paid for more flexible error control.
Due to their property of valid coverage based on \emph{arbitrary number} of
looks at the data, $k < 1$ support interval will also typically be wider than
$(1 - k)\%$ confidence intervals adjusted via group sequential or adaptive trial
methodology which are more fine-tuned to specific interim analysis strategies
\citep{Wassmer2016}. These strategies are, however, typically more restrictive
and computationally involved compared to the flexible and easily computable
$k < 1$ support intervals which we present here.
...
% they do not require specification of the
...
% performed (\eg{} as for alpha spending functions).
% This makes support intervals
% a suitable tool when evidence quantification is the highest priority for
...
% Since the coverage of confidence intervals is not controlled when
% repeated looks at accumulating data are performed, the same also holds true for
% minimum support intervals.
Minimum support intervals are thus only useful for giving confidence intervals
an evidential interpretation, but a $k$ minimum support interval with $k < 1$,
itself does not provide $(1 - k)\%$ coverage under optional stopping.
