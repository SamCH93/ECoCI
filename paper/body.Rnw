...
\begin{figure*}[!htb]
<< "figure-BFfun-RECOVERY", fig.height = ifelse(type == "journal", 3.25, 3.5) >>=
...
\caption{Application of support intervals to data from the RECOVERY trial
  ...
  compared to usual
  care. %standard error $\sigma = 0.05$. The Bayes factor for testing
  $\h{0}\colon \theta = \theta_0$ versus $\h{1}\colon \theta \neq \theta_0$ is
  shown as a function of the null value $\theta_0$. A normal distribution
  centered around the clinically relevant proportional mortality reduction
  $\mu_\theta = \log(0.8) = -0.22$ (as deemed by the trial steering committee)
  with unit-standard deviation $\sigma_\theta = 2$ \citep[ch.
  2.4.2]{Spiegelhalter2004} is used as a prior for $\theta$ under the
...
\begin{table*}[!h]
  ...
  \caption{Classifications of evidence for $\h{0}$ provided by $\BF_{01} = k$.
    ...
    $\sqrt{10}$ as suggested by \citet{Kass1995}. \citet{Fisher1956} defined his
    cutoffs relative to the likelihood of the data under the maximum likelihood
    estimate of the parameter $\theta$. He only named the $k < 1/15$ category,
    for the other categories the names from \citet{Shafer2021} are shown.}
...
class of prior distributions leads to so-called \emph{minimum support sets}
which, for a given class of priors, have a one-to-one mapping with confidence
sets and thus give them an evidential interpretation
(Section~\ref{sec:SIbounds}). We then illustrate how the sample size of a future
study can be determined based on support, which provides an alternative to the
conventional approaches based on either power or precision
...
Denote by $\that$ an asymptotically normal estimator of a univariate unknown
...
effective sample size and $\lambda^2$ is a unit variance. An approximate
$(1 - \alpha)100\%$ confidence interval for $\theta$ is given by
\begin{align}
    \label{eq:ci}
...
\begin{align}
  ...
  %  = \sqrt{1 + \sigma_\theta^2/\sigma^2} \, \exp\left[-\frac{1}{2}\left\{\frac{(\that -
...
\begin{align}
    \label{eq:si}
    \that \pm \sigma \times
    \sqrt{\log\left(1 + \sigma_\theta^2/\sigma^2\right) + \frac{(\that - \mu_\theta)^2}{\sigma^2 + \sigma_\theta^2} -
    2\log k}.
\end{align}
...
prior), \ie{} the standard error $\sigma$ has to be sufficiently small relative
to the prior standard deviation $\sigma_\theta$. In the following, we will
discuss how different prior means $\mu_\theta$ and standard deviations
$\sigma_\theta$ affect the resulting support intervals.

When the prior standard deviation decreases ($\sigma_\theta \downarrow 0$), the
prior approaches a point mass at $\mu_\theta$. The width of the support interval
is then fully determined by the difference between the parameter estimate
$\that$ and the prior mean $\mu_\theta$ divided by the standard error $\sigma$,
smaller differences leading to tighter support intervals. In contrast, for
priors that become increasingly diffuse ($\sigma_\theta \to \infty$), the
$k \geq 1$ support interval~\eqref{eq:si} extends to the entire real number
line, indicating that all values $\theta \in \R$ receive more support from the
data than the diffuse alternative, regardless of the location of the prior mean
$\mu_\theta$. This particular behavior provides another perspective on the
well-known Jeffreys-Lindley paradox \citep{Wagenmakers2021a}; the confidence
interval from~\eqref{eq:ci} only spans a finite range around the parameter
estimate $\that$, so that the corresponding null hypothesis significance tests
would reject the parameter values outside, whereas for the same values the Bayes
factor would indicate evidence for the null hypothesis. Finally, centering the
prior around the parameter estimate ($\mu_\theta = \that$) and setting the
variance equal to the variance of one effective observation
($\sigma_\theta^2 = n \times \sigma^2$ with $n$ the effective sample size),
produces the support interval for Jeffreys's approximate Bayes factor
\citep{Wagenmakers2022}. In this case, the standard error multiplier has a
...
respective support interval with $k \geq 1$ to exist.
...
specification of a prior mean and standard deviation. A different approach is to
use a so-called \emph{local prior}, that is, a unimodal and symmetric prior
centered around the null value $\theta_0$ \citep{Berger1987b}, typically
requiring only the specification of a scale parameter.
...
\begin{align}
    \label{eq:silocal}
    \that \pm \sigma \times
    \sqrt{\left\{\log(1 + \sigma_\theta^2/\sigma^{2}) - 2\log k \right\}
    (1 + \sigma^{2}/\sigma_\theta^2)}.
\end{align}
...
is always the same under the two-parameter normal prior approach.
% The local normal prior for $\theta$ under the alternative depends itself on
% the null value $\theta_0$ resulting in a different support interval compared
% to the one based on the two-parameter normal prior\eqref{eq:si}.
% interpretation of the support interval based on local normal
...
% likelihood for each parameter value.
To fully specify the support interval~\eqref{eq:silocal}, the prior standard
deviation $\sigma_\theta$ needs to be chosen. One standard choice is to set it
equal to the variance of a single observation
($\sigma_\theta^2 = n \times \sigma^2$), known as unit-information prior
\citep{Kass1995b}. This approach leads to the $k$ support interval
\begin{align}
    \label{eq:silocalui}
    \that \pm \sigma \times
...
\emph{normal moment priors} $\theta \sim \mathrm{NM}(m, \sigma_\theta^2)$, which
have density
$f(\theta \given \mu_\theta, \sigma_\theta^2) = \Nor(\theta \given \mu_\theta, \sigma_\theta^2) \times (\theta - \mu_\theta/\sigma_\theta^2$
where $\Nor(\cdot \given \mu_\theta, \sigma_\theta^2)$ denotes the density
function of a normal distribution with mean $\mu_\theta$ and variance
...
\begin{align}
  ...
  % \left(1 + \sigma^2/\sigma_\theta^2\right)}
  %% uncomment for short equation
  \sqrt{\dfrac{\left[2 \lw{0}\left\{\dfrac{(1 + \sigma_\theta^2/\sigma^2)^{3/2}
  \sqrt{e}}{2 k}\right\} - 1\right]\,\sigma_\theta^2}{
  (1 + \sigma_\theta^2/\sigma^2)\,\sigma^2}}
\end{align}
...
that this situations occurs when
$(1 + \sigma_\theta^2/\sigma^2)^{3/2}/(2k) < \sqrt{e}$, meaning that the
standard error $\sigma$ has to be sufficiently small relative to the prior scale
$\sigma_\theta$ parameter and the support level $k$, so that the interval
exists.
...
in the sense that assigning a point prior at the observed parameter estimate
hardly reflects prior knowledge about $\theta$ but can rather be considered
cheating \citep{Berger1987}. This is reflected by the fact that for a fixed
...
variance in the Bayes factor~\eqref{eq:bf01local} leads to
...
the minimum Bayes factor over the class of local normal priors
% %% uncomment for long equation
% \begin{align}
%   \minBF_{01}(\that ; \theta_0) =
%   \begin{cases}
...
confidence intervals for the local normal prior calibration. Similarly, as the
mapping between Bayes factor bounds and $p$-values \citep{Held2018} provides an
evidential interpretation of $p$-values, the mapping displyed in
Figure~\ref{fig:mappingCIk}
% parallels the
% mapping between Bayes factor bounds and $p$-values \citep{Held2018}, and it also gives
provides an evidential interpretation of confidence intervals, uncoupling them
from their long-term frequency interpretation.
...
minimum support intervals are fully specified and can be readily
computed. %(row five to seven in Figure~\ref{fig:comparison}).
For the normal, local normal, and the nonlocal normal moment prior
%(row two to four in Figure~\ref{fig:comparison}),
we choose their parameters as follows. The trial steering committee determined
the sample size of the trial based on an assumed clinically relevant log hazard
ratio of $\log \Sexpr{exp(m)} = \Sexpr{round(m, 2)}$. This effect size can be
used to inform the normal prior under the alternative $\h{1}$, \ie{} we specify
the mean $\mu_\theta = \Sexpr{round(m, 2)}$ along with the unit-information
variance $\sigma_\theta^2 = \Sexpr{round(v, 2)}$ for a log hazard ratio
\citep[ch. 2.4.2]{Spiegelhalter2004}. Likewise, we use the unit-information
variance $\sigma_\theta^2 = \Sexpr{round(v, 2)}$ as the variance of the local
normal prior. The scale parameter of the nonlocal moment prior $s$ is elicited
with a similar approach as \citet{Pramanik2022}; The value
...
% receive a desired amount of probability. For instance, \citet{Pramanik2022}
% choose $s = 0.2$ so that about 85\% of the probability mass is assigned to
% effect sizes in $(0.2, 0.8)$ and $(-0.8, -0.2)$, representing the typical
% range of standardized mean difference effect sizes in psychological research.
...
\caption{Comparison of different support intervals for data from RECOVERY trial.
  ...
  mortality reduction $\mu_{\theta} = \log \Sexpr{exp(m)} = \Sexpr{round(m, 2)}$ (as deemed
  by the trial steering committee) and has unit-standard deviation
  $\sigma_\theta = \Sexpr{round(sqrt(v), 2)}$. The local normal prior also has
  unit-standard deviation of $\sigma_\theta = \Sexpr{round(sqrt(v), 2)}$. The
  scale parameter of the nonlocal normal moment prior is
  $\sigma_\theta = \Sexpr{round(snonlocal, 2)}$ so that
  \Sexpr{round(100*pnonlocal, 2)}\% of the probability is assigned to log hazard
  ratios between $(\theta_0 - \log 2, \theta_0)$ and
  $(\theta_0, \theta_0 + \log 2)$, representing effect sizes that at most double
...
overoptimistic assessment of support, in the same way that Bayes factor bounds
provide an overoptimistic quantification of evidence.
...
\label{sec:design}
The sample size of a future study is typically derived from (i) a targeted power
of a hypothesis test or (ii) a targeted precision of a future
confidence/credible interval. Here, we provide an alternative where the sample
size of a future study is determined to have a desired level of support.
...
\begin{align}
    \label{eq:existence}
    \log(1 + \sigma_\theta^2/\sigma^2) + \frac{(\that - m)^2}{\sigma^2 + \sigma_\theta^2} \geq 2 \log k.
\end{align}
...
the coverage of the corresponding $k$ support set $\mbox{S}_k$ with $k < 1$ is
bounded by
\begin{align}
...
A $(1 - k)\%$ confidence interval obtained from a $k$ support interval will
usually be wider than an unadjusted standard $(1 - k)\%$ confidence interval.
This is the price that has to be paid for more flexible error control. Due to
their property of valid coverage based on arbitrary number of looks at the data,
confidence intervals obtained from support intervals will also typically be
...
% We are not claiming that support intervals are more efficient in controlling
% error rates than other types of adjustment methods, but
are flexible and easy to implement.
% they do not require specification of the number of interim analyses beforehand
% (\eg{} as for Pocock or O'Brien Fleming corrections) or the recalculation of
% the alpha level when an interim analysis is performed (\eg{} as for alpha
% spending functions).
This makes support intervals a suitable tool when evidence quantification is the
...
which violates the assumption of the universal bound.
% Since the coverage of confidence intervals is not controlled when repeated
% looks at accumulating data are performed, the same also holds true for minimum
% support intervals.
Minimum support intervals are thus only useful for giving confidence intervals
an evidential interpretation, but not for obtaining confidence intervals that
have controlled coverage under optional stopping.
...
\begin{table*}[!htb]
    ...
    \caption{Summary of confidence intervals (CI), support intervals (SI), and
      ...
      confidence level $(1 - \alpha)$ or the support level $k$. For the support
      intervals, the standard error multipliers M additionally depend on the
      parameters of the prior for $\theta$ under the alternative hypothesis:
      mean $\mu_\theta$ and standard deviation $\sigma_\theta$ for the normal
      prior, standard deviation $\sigma_\theta$ for the local normal prior, and
      scale $\sigma_\theta$ for the nonlocal normal moment prior. The quantile
      ...
      respective (minimum) support intervals only exist for support levels $k$
      for which the standard error multiplier is real-valued, \ie{} the
      respective term in the square root needs to be non-negative and/or the
      argument for $\lw{-1}(\cdot)$ needs to be in $[-1/e, 0)$. All interval
      types can be computed with the R package \texttt{ciCalibrate}
    ...
    $\surd \{\log\left(1 + \sigma_\theta^2/\sigma^2\right) + (\that - \mu_\theta/(\sigma^2 + \sigma_\theta^2) -
    2\log k\}$ \\
    ...
    $\surd ([2 \lw{0}\{(1 + \sigma_\theta^2/\sigma^2)^{3/2}/(2 k e^{-1/2})\} - 1]\{1 + \sigma^2/\sigma_\theta^2\})$ \\
...
% This fact is also illustrated through the proposed minimum support intervals
% which can be one-to-one mapped to bits of refutational surprisal (since both
% can be mapped to confidence levels); without a specific alternative hypothesis
% only the maximum evidence \emph{against} the included parameter values can be
% quantified.
...
\citep{Kass1995, OHagan2004, Gneiting2007a}. Such ``descriptive inferential
statistics'' are especially important for the analysis of convenience data
samples which typically violate assumptions of the underlying statistical model
% , so that overconfident error-rate statements are not appropriate
