...
normal likelihood (Section~\ref{sec:SInormality}). Our main results are
easy-to-use formulas for computing support intervals that only require summary
...
the confidence level $(1 - \alpha)\times 100\%$ and standard error $\sigma$, the
...
\begin{align}
  ...
  \left(1 + \frac{\sigma^2}{\sigma_\theta^2}\right)}
\end{align}
with $\lw{0}(\cdot)$ denoting the principal branch of the Lambert W function, see
Appendix~\ref{app:lambertW} for more details. It is possible that the support
...
level $(1 - \alpha)\times 100\%$ of the approximate confidence interval~\eqref{eq:ci}, see
...
@
\caption{Mapping between confidence level $(1 - \alpha)\times100\%$ and minimum
...
construction of $(1 - k)100\%$ approximate confidence interval via the the
...
\begin{table*}[!htb]
    ...
    \caption{Summary of confidence intervals (CI), support intervals (SI), and
      ...
      that satisfies $\mbox{W}(y) < 1$ for $y \in (-1/e, 0)$, see
      Appendix~\ref{app:lambertW} for more details. The respective (minimum)
...
% are overconfident
\citep{Amrhein2019, Shafer2021, Greenland2022}. In fact, even one of the best
...
of the manuscript. Our acknowledgement of these individuals does not imply their
