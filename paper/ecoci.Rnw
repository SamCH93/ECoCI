...
$\that$ could be an estimated regression coefficient from generalized linear
model and $\sigma$ its standard error. In many simple settings, the standard
error is of the form $\sigma = \lambda/\sqrt{n}$, where $\lambda^2$ is the
...
by so-called \emph{nonlocal priors}. These priors are characterized by having zero
...
with symmetry point $\theta_0$ and spread $\sigma_\theta$ which have density
...
\begin{align}
  ...
  \left(1 + \frac{\sigma^2}{\sigma_\theta^2}\right)}
\end{align}
with $\lw{0}(\cdot)$ denoting the principal branch of the Lambert W function. The Lambert W function is the (complex) multivalued function $\text{W}(\cdot)$ satisfying $\text{W}(x)
...
0)$, see \citet{Corless1996} for more details. It is possible that the support interval~\eqref{eq:sinonlocal} is empty, as for the other two types of support intervals. This happens when the Lambert W term is smaller than one half so that the square root is undefined. Since $\lw{0}(\Sexpr{round(lArg, 2)}) \approx 1/2$, this situations occurs when
...
to the prior spread parameter $\sigma_\theta$ and the support level $k$, so that
...
\caption{Comparison of prior distributions for the parameter $\theta$ under the
  alternative $\h{1}$ in terms of resulting support interval width and highest
  level for which it is non-empty. A data model
  $\that \given \theta \sim \Nor(\theta, \lambda^{2}/n = \Sexpr{unitvar}/n)$ is
  assumed in all cases. The prior scale/spread parameter is set to
  $\sigma_{\theta} = \Sexpr{sqrt(unitvar)}$. The normal prior (correct mean) has
  mean equal to the parameter estimate $\that$, while the normal prior (wrong
  mean) has mean one standard deviation $\lambda = \Sexpr{sqrt(unitvar)}$ away
  from $\that$.}
...
can be compared in terms of their width (Figure~\ref{fig:priorComparison} top)
as a function of the sample size $n$. For small sample sizes, the normal
(correct mean) prior produces the narrowest support intervals, followed by the
local normal, the normal (wrong mean), and the nonlocal normal moment prior.
Thus, a well-chosen normal prior can increase the precision of support
inference, whereas a poorly chosen normal prior can decrease precision. However,
the differences in width between the priors mostly disappear with increasing
sample size. In the realistic range between 10 and a few hundred samples, the
local normal prior seems to be a reasonable choice, as it leads to support
intervals almost as narrow as the normal (correct mean) prior, without the need
to specify a mean.
...
$n$, the highest levels from the normal and local normal priors are similar and
show the same growth rates, while the level from the nonlocal moment prior is
higher and grows much faster. This is expected because nonlocal priors are
specifically designed to produce Bayes factors with faster accumulation of
evidence for the null hypothesis. Although nonlocal moment priors result in
wider support intervals than the other priors, for small sample sizes, they may
be the only prior type that can even produce a support interval at, say,
Jeffreys' strong evidence level $k = 10$.
