...
\begin{center}
  \begin{minipage}{14cm} {\small
      ...
      {\centering \textbf{Abstract} \\
        ...
        with confidence intervals. We also illustrate how the sample size of a
        ...
        data from a clinical trial illustrates how support intervals can lead to
...
\begin{table*}[!htb]
    ...
    \caption{Classifications of evidence for $\h{0}$ provided by $\BF_{01} = k$.
      ...
      $\sqrt{10}$ as suggested by \citet{Held2018}. \citet{Royall1997}
      calibrated his classification based on the number of consecutive successes
      in a series of Bernoulli trials with success probability $\theta$ and
      contrasting $\h{0}\colon \theta = 0.5$ to $\h{1}\colon \theta = 1$.
      \citet{Fisher1956} defined his cutoffs relative to the likelihood of the
      data under the maximum likelihood estimate of the parameter $\theta$. He
      only named the $k < 1/15$ category.}
...
does not exist, as the term below the square root in~\eqref{eq:si} may become
...
Bayes factor \citep{Wagenmakers2022} which is equal to well-known approximation
of the Bayes factor based on the Bayesian information criterion
...
\begin{align}
  ...
  \left(1 + \frac{\sigma^2}{\sigma_\theta^2}\right)}
\end{align}
with $\lw{0}(\cdot)$ denoting the principal branch of the Lambert W
function.\footnote{The Lambert
W function is the (complex) multivalued function $\text{W}(\cdot)$ satisfying $\text{W}(x)
\, \exp\{\text{W}(x)\} = x$. For real $x$, it is defined for $x \in [-1/e,
\infty)$. For $x \geq
...
0)$, see \citet{Corless1996} for more details.} It is possible that the support
...
possible among all possible support intervals. When minimizing over the class of
all two-parameter normal priors, i.e., the Bayes factor~{\eqref{eq:bf01}}, we
also obtain the same minimum Bayes factor~{\eqref{eq:minBFsimple}} and
consequently the same minimum support interval~\eqref{eq:sisimple}.
...
mappings displayed in Figure~\ref{fig:mappingCIk} provides confidence intervals
an evidential interpretation. Specifically, it enhances their long-term
...
\begin{table*}[!htb]
    ...
    \caption{Summary of confidence intervals (CI), support intervals (SI), and
      ...
      that satisfies $\mbox{W}(y) < 1$ for $y \in (-1/e, 0)$. The respective (minimum)
      support intervals only exist for support levels $k$ for which the standard
      error multiplier is real-valued, \ie{} the respective term in the square
      root needs to be non-negative and/or the argument for $\lw{-1}(\cdot)$
      needs to be in $[-1/e, 0)$. All interval types can be computed with the R
      package \texttt{ciCalibrate} (Appendix~\ref{app:Rpkg}).}
    \label{tab:summary}
    \begin{tabular}{lll}
    \toprule
...
% are overconfident
\citep{Amrhein2019, Shafer2021}. In fact, even one of the best
known proponents of $p$-values -- R.A. Fisher -- noted ``For all purposes, and
more particularly for the \emph{communication} of the relevant evidence supplied
by a body of data, the values of the Mathematical Likelihood are better fitted
to analyse, summarize, and communicate statistical evidence of types too weak to
supply true probability statements'' \citet[p. 70]{Fisher1956} clearly
...
of the manuscript. Our acknowledgment of these individuals does not imply their
...
<< "code-snippet-ciCalibrate", echo = TRUE, fig.height = 3.5, size = codeFont >>=
